...
class MultiPartXmlBibleProvider extends IBibleProvider {
  ...
  /* Initialize Bible Directory Path and load document */
  @override
  Future init() async {
  ...
  /* Add books to directory from the "books" label in the XML document */
  @override
  ...
  Chapter _convertChapterFromXml(xml.XmlElement item) {
    ...
    List<IChapterElement> elements = _getChapterElements(item);
  ...
  List<IChapterElement> _getChapterElements(xml.XmlElement node) {
    ...
    for (var child in node.children) {
      ...
      if (child.children.length > 0 &&
        ...
        convertedElement.elements.addAll(_getChapterElements(child));
      }
